...
class MusicTest(commands.Cog, name='Music'):
    """
    ...
    def play_next(self, ctx):
        ...
        if len(self.song_queue[ctx.guild]) > 1:
            ...
            voice.play(FFmpegPCMAudio(self.song_queue[ctx.guild][0]['source'], **MusicTest.FFMPEG_OPTIONS), after=lambda e: self.play_next(ctx))
            voice.is_playing()
        else:
    ...
    async def play(self, ctx, *, video: str):
        ...
        voice = get(self.bot.voice_clients, guild=ctx.guild)
        song = MusicTest.search(ctx.author.mention, video)
        ...
        if not voice.is_playing():
            ...
            voice.play(FFmpegPCMAudio(song['source'], **MusicTest.FFMPEG_OPTIONS), after=lambda e: self.play_next(ctx))
            voice.is_playing()
        else:
